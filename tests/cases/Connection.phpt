...
class ConnectionTest extends Tester\TestCase
{
	...
	public function testGetUrl()
	{
		...
		Tester\Assert::same(Connection::PAYU_URL . '/UTF/Payment/get/xml',
			$connection->getUrl(new PaymentInfoRequest()));

		$connection = new Connection(new Config(12345,
				'ab',
				'cd',
				'ef',
				Config::ENCODING_ISO_8859_2,
				Config::FORMAT_TXT));
		Tester\Assert::same(Connection::PAYU_URL . '/ISO/Payment/cancel/txt',
			$connection->getUrl(new PaymentCancelRequest()));

		$connection = new Connection(new Config(12345, 'ab', 'cd', 'ef', 'uTf', 'xML'));
		Tester\Assert::same(Connection::PAYU_URL . '/UTF/Payment/cancel/xml',
			$connection->getUrl(new PaymentCancelRequest()));
	}
